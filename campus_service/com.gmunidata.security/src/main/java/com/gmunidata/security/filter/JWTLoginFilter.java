...
public class JWTLoginFilter extends UsernamePasswordAuthenticationFilter {
    ...
    private CustomProviderManager authenticationManager;

    private UserService userService;

    public JWTLoginFilter(AuthenticationManager authenticationManager) {
        this.authenticationManager = (CustomProviderManager)authenticationManager;
    }
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            auth = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(username, password, new ArrayList<>()), request.getHeader("requestType"));
        } catch (Exception e) {
            throw e;
        }
        return auth;
    ...
    private Map getRequestPayload(HttpServletRequest req) {
        ...
        try {
            Map<String,Object> mp = JsonUtil.json2Object(sb.toString(),Map.class);
            return mp;
        } catch (Exception e) {
            e.printStackTrace();
        }
