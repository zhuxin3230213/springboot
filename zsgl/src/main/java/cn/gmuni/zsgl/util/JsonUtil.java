...
public class JsonUtil {
    ...
    static {
        ...
        //设置输入时忽略JSON字符串中存在的而java对象实际没有的属性
    ...
    public static <T> List<T> listJson2ListObject(List<String> jsons, Class<T> tClass) {
        if (jsons == null) {
            return null;
        }
        List<T> ts = new ArrayList<T>();
        for (String j : jsons) {
            ts.add(JsonUtil.json2Object(j, tClass));
        }
        return ts;
    }
